...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if($foto != "") {
        ...
        if(in_array($ekstensifoto, $ekstensifotoV) === true)  {
            $FotoB = move_uploaded_file($tmpName, 'asset/img/'. $fotoB); //memindah file gambar ke folder gambar
            ...
            echo "<script>alert('Pilih Gambar Terlebih dahulu.');window.location='ruangan.php';</script>";
    }
    if($foto2 != "") {
        $ekstensifotoV = array('png','jpg','jpeg'); //ekstensi file gambar yang bisa diupload 
        ...
        if(in_array($ekstensifoto, $ekstensifotoV) === true)  {
              $FotoD = move_uploaded_file($tmpName, 'asset/img/'. $fotoA); //memindah file gambar ke folder gambar
                      // jalankan query UPDATE berdasarkan ID yang produknya kita edit
                } else {     
                 //jika file ekstensi tidak jpg dan png maka alert ini yang tampil
                    echo "<script>alert('Ekstensi gambar yang boleh hanya jpg atau png.');window.location='ruangan.php';</script>";
    ...
    // periska query apakah ada error
        if(!$result){
            ...
            echo "<script>alert('Data berhasil disimpan.');window.location='ruangan.php';</script>";
        }
    }else{
        echo "<script>alert('Data gagal disimpan.');window.location='ruangan.php';</script>";
    }
}
?>
